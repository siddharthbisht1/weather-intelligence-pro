...
def get_eco_insights(city: str):
    # Bina kisi password/token ke data bhejega (No 401 Error!)
    return {
...
# Yahan dependencies=[Depends(...)] NAHI hona chahiye
@router.get("/emergency-alerts/{city}")
def get_emergency_alerts(city: str):
    import random
    
    # 📍 City ke asli coordinates ka dictionary
    city_data = {
    ...
    # Agar city list mein hai toh exact location lega, warna fallback central India dega
    city_lower = city.lower()
    if city_lower in city_data:
        base_lat, base_lng = city_data[city_lower]
    else:
        # Unknown city ke liye random
        base_lat = random.uniform(20.0, 28.0)
        base_lng = random.uniform(72.0, 80.0)
    
    return {
        "city": city.capitalize(),
        "coords": [base_lat, base_lng], # Asli Map center!
        "kpi": {
        ...
        "map_markers": [
            # Offset chota kar diya (-0.05 to 0.05) taaki marker city ke andar hi dikhe, bahar na bhage
